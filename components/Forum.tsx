...
const ForumPost: React.FC<{ author: User | undefined; content: string; date: string; title?: string; isOp?: boolean }> = ({ author, content, date, title, isOp }) => {
    const safeDate = date ? String(date) : new Date().toISOString();
    const formatDate = (dateStr: string) => {
        try {
            return new Date(dateStr).toLocaleString('en-US', { month: '2-digit', day: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' });
        } catch (e) {
            return 'Unknown Date';
        }
    };
    ...
    return (
        <div className={`flex flex-col md:flex-row border-b border-slate-900/10 ${isOp ? 'bg-indigo-50/20' : 'bg-white'}`}>
            {/* User Sidebar */}
            <div className="md:w-48 bg-slate-50 p-6 flex flex-col items-center text-center shrink-0 border-r border-slate-200">
                <div className="relative mb-3 group">
                    <img src={author?.avatarUrl || 'https://picsum.photos/100'} className="w-20 h-20 rounded bg-white border-2 border-slate-200 object-cover shadow-sm" alt="Avatar" />
                    {author?.isAdmin && <div className="absolute -top-2 -right-2 bg-purple-600 text-white p-1 rounded shadow-sm" title="Admin"><Shield className="w-3 h-3" /></div>}
                </div>
            ...
            <div className="flex-1 flex flex-col min-h-[200px]">
                {/* Header */}
                <div className="bg-slate-100/50 px-6 py-3 border-b border-slate-100 flex justify-between items-center h-12">
                     <div className="text-[10px] text-slate-400 font-mono font-bold uppercase flex items-center gap-3">
                        {isOp ? <span className="bg-indigo-600 text-white px-2 py-0.5 rounded text-[9px]">OP</span> : <span>REPLY</span>}
                        <span className="w-px h-3 bg-slate-300"></span>
                        <Clock className="w-3 h-3" /> {formatDate(safeDate)}
                     </div>
                     <div className="text-[10px] font-mono font-bold text-slate-300">#{safeDate.length > 4 ? safeDate.substring(safeDate.length-4) : '0000'}</div>
                </div>

                {/* Body */}
                <div className="p-8 flex-1 text-slate-800 text-sm leading-relaxed font-sans">
                    {title && <h2 className="text-xl font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">{title}</h2>}
...
export const Forum: React.FC<ForumProps> = ({ currentUser, users, threads, onAddThread, onAddReply, onRequireAuth }) => {
  ...
  const formatDate = (dateStr: string) => {
    try {
        return new Date(dateStr).toLocaleString('en-US', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
    } catch (e) {
        return 'Unknown';
    }
  };
  ...
  if (activeView === 'detail' && selectedThread) {
    return (
      <div className="max-w-7xl mx-auto min-h-screen pb-12 pt-4">
        ...
        <div className="bg-white rounded-t-xl rounded-b-xl shadow-hard border-2 border-slate-900 overflow-hidden">
             ...
             <div className="bg-slate-900 text-white px-6 py-4 font-bold text-sm uppercase flex justify-between items-center border-b border-slate-700">
                 <div className="flex items-center gap-2">
                    ...
                    <span>TRANSMISSION_LOG // {String(selectedThread.id).substring(0,8)}</span>
                 </div>
